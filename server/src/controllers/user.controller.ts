...
export const updateShopAvatar = asyncHandler(
    async (req: IGetUserAuthInfoRequest, res: Response, next: NextFunction) => {
        ...
        if (req.file) {
            if (avatar?.public_id) {
                await deleteTheOldPicture(avatar.public_id);
            }
            const file = req.file;
            ...
            seller = await Shop.findByIdAndUpdate(id, {
                avatar: {
                    public_id: uploadedOnCloudinary.public_id,
                    url: uploadedOnCloudinary.secure_url
                }
        ...
        res
            .status(200)
            .json(new ApiResponse(200, "User avatar updated successfully", seller));
...
export const updateShop = asyncHandler(
    async (req: IGetUserAuthInfoRequest, res: Response, next: NextFunction) => {
        // Get shop updated data from user
        const {shopName, email, shopDescription, phoneNumber, shopAddress, zipcode} = req.body;
        ...
        if (seller) {
            ...
            seller.description = shopDescription
            seller.email = email
            seller.phoneNumber = phoneNumber
            seller.address = shopAddress
        ...
        res
            .status(200)
            .json(new ApiResponse(200, "shop updated successfully.", seller));
